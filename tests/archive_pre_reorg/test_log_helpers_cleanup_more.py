...
def test_write_json_full_failure_fallback(tmp_path, monkeypatch):
    ...
    # Provide an object that json.dumps can't serialize by default
    obj = BadJSON()

    # Expect write_json to try fallback serialization then call write_text which we've mocked to raise
