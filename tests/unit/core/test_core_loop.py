...
def test_ensure_runtime_checks_monkeypatch(monkeypatch):
    ...
    monkeypatch.setattr(
        "infra_monitoring.core.core.ensure_log_dirs_exist", lambda: (_ for _ in ()).throw(Exception("boom"))
    )
    monkeypatch.setattr(
        "infra_monitoring.core.core.ensure_last_ts_exists", lambda: (_ for _ in ()).throw(Exception("boom"))
    )
    # should not raise
...
def test_collect_and_emit_handles_collect_exceptions(monkeypatch):
    ...
    monkeypatch.setattr("infra_monitoring.core.core._collect_metrics", lambda: (_ for _ in ()).throw(Exception("bad")))
    # monkeypatch emitter to be noop
    monkeypatch.setattr("infra_monitoring.core.core._emit_snapshot", lambda s, r, v: None)
