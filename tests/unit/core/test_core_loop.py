...
core = importlib.import_module("infra_monitoring.core.core")
...
def test_ensure_runtime_checks_monkeypatch(monkeypatch):
    ...
    monkeypatch.setattr(
        "infra_monitoring.core.core.ensure_log_dirs_exist",
...
def test_collect_and_emit_handles_collect_exceptions(monkeypatch):
    ...
    monkeypatch.setattr(
        "infra_monitoring.core.core._collect_metrics",
        lambda: (_ for _ in ()).throw(Exception("bad")),
    )
    # monkeypatch emitter to be noop
    monkeypatch.setattr(
        "infra_monitoring.core.core._emit_snapshot", lambda s, r, v: None
    )
...
def test_run_loop_one_cycle(monkeypatch):
    ...
    monkeypatch.setattr(
        "infra_monitoring.core.core._ensure_runtime_checks", lambda: None
    )
    monkeypatch.setattr(
        "infra_monitoring.core.core._collect_and_emit", lambda s, v: {"state": "S"}
    )
    monkeypatch.setattr(
        "infra_monitoring.core.core._run_maintenance",
