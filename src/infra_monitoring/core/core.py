...
def run_loop(interval: float, cycles: int, verbose_level: int) -> None:
    ...
    if interval < 0.1:
        logging.getLogger(__name__).warning(
            "Interval too small (%.2fs). Recommended >= 0.1s to avoid overload",
            interval,
        )
    ...
    try:
        while True:
            ...
            if cycle_elapsed > 1.0:
                logging.getLogger(__name__).warning(
                    "Slow cycle: %.2fs (expected < 1.0s)", cycle_elapsed
                )
            now = time.monotonic()
            try:
                last_rotate, last_compress, last_safe_remove, last_hourly = (
                    _run_maintenance(
                        now,
                        last_rotate,
                        last_compress,
                ...
                logging.getLogger(__name__).debug(
                    "Failed to schedule maintenance: %s", exc, exc_info=True
                )
            executed += 1
            ...
            if interval > 0.0:
                try:
                    ...
                    logging.getLogger(__name__).debug(
                        "Sleep interrupted: %s", exc, exc_info=True
                    )
...
def _ensure_runtime_checks() -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            "ensure_log_dirs_exist failed: %s", exc, exc_info=True
        )
    try:
        ...
        logging.getLogger(__name__).debug(
            "ensure_last_ts_exists failed: %s", exc, exc_info=True
        )
...
def _collect_and_emit(state: SystemState, verbose_level: int) -> dict:
    ...
    try:
        ...
        try:
            _expose_metric(
                "app_collect_errors_total",
                float(_APP_COLLECT_ERRORS),
                "Number of collection errors",
            )
        except Exception:
            logging.getLogger(__name__).debug(
                "Failed to expose collect error metric", exc_info=True
            )
        ...
        try:
            from infra_monitoring.services.monitoring.handlers import (
                network_learning_handler,
            )
            ...
            logging.getLogger(__name__).debug(
                "Failed to record daily network learning: %s", exc, exc_info=True
            )
    ...
    try:
        _expose_metric(
            ...
            float(_APP_LOOP_ITERATIONS),
            "Loop iterations counter (approx)",
        )
    except Exception:
        logging.getLogger(__name__).debug(
            "Failed to expose loop iterations metric", exc_info=True
        )
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            "Failed to expose last success timestamp metric", exc_info=True
        )

    _emit_snapshot(
        snap_to_emit if isinstance(snap_to_emit, dict) else None, result, verbose_level
    )

    # Log de ciclo completo em modo verbose
    if verbose_level >= 2:
        logging.getLogger(__name__).info(
            "Cycle complete: %d metrics collected, state=%s", len(metrics), state_name
        )
